...
///
/// Resolver is a client for obtaining public Rustweave wRPC URL.
///
/// Resolver queries a list of public Rustweave Resolver URLs using HTTP to fetch
...
#[wasm_bindgen]
impl Resolver {
    /// List of public Rustweave Resolver URLs.
    #[wasm_bindgen(getter)]
    pub fn urls(&self) -> ResolverArrayT {
        Array::from_iter(self.resolver.urls().iter().map(|v| JsValue::from(v.as_str()))).unchecked_into()
    }

    /// Fetches a public Rustweave wRPC endpoint for the given encoding and network identifier.
    ...
    /// Fetches a public Rustweave wRPC endpoint URL for the given encoding and network identifier.
    ...
    /// Connect to a public Rustweave wRPC endpoint for the given encoding and network identifier
