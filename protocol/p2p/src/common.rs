use crate::{convert::error::ConversionError, core::peer::PeerKey, RustweavedMessagePayloadType};
...
pub enum ProtocolError {
    ...
    UnexpectedMessage(&'static str, Option<RustweavedMessagePayloadType>),
    ...
    IncomingRouteCapacityReached(RustweavedMessagePayloadType, String),
    ...
    NoRouteForMessageType(RustweavedMessagePayloadType),
...
/// Wraps an inner payload message into a valid `RustweavedMessage`.
/// Usage:
/// ```ignore
...
macro_rules! make_message {
    ...
    ($pattern:path, $msg:expr, $response_id:expr, $request_id: expr) => {{
        $crate::pb::RustweavedMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: $request_id }
    }};
}
...
macro_rules! make_response {
    ($pattern:path, $msg:expr, $response_id:expr) => {{
        $crate::pb::RustweavedMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: 0 }
    }};
}
...
macro_rules! make_request {
    ($pattern:path, $msg:expr, $request_id:expr) => {{
        $crate::pb::RustweavedMessage { payload: Some($pattern($msg)), response_id: 0, request_id: $request_id }
    }};
}

/// Macro to extract a specific payload type from an `Option<pb::RustweavedMessage>`.
/// Usage:
/// ```ignore
...
/// Macro to await a channel `Receiver<pb::RustweavedMessage>::recv` call with a default/specified timeout and expect a specific payload type.
/// Usage:
/// ```ignore
...
/// Macro to indefinitely await a channel `Receiver<pb::RustweavedMessage>::recv` call and expect a specific payload type (without a timeout).
/// Usage:
/// ```ignore
