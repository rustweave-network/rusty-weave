...
use kaspa_wrpc_server::{
    ...
    router::Router,
    server::Server,
    service::{RustweaveRpcHandler, Options},
};
use result::Result;
use std::sync::Arc;
use workflow_log::*;
...
async fn main() -> Result<()> {
    ...
    let rpc_handler = Arc::new(RustweaveRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("Rustweave wRPC server is listening on {}", options.listen_address);
