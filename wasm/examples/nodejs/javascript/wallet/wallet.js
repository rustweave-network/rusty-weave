    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToRustweaveString(value(tx)||0)
            });
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        list.push({
                            ...
                            Mature: sompiToRustweaveString(data.balance.mature),
                            Pending: sompiToRustweaveString(data.balance.pending),
                            Outgoing: sompiToRustweaveString(data.balance.outgoing),
