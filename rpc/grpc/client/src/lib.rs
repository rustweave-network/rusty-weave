...
use kaspa_grpc_core::{
    ...
    protowire::{rustweaved_request, rpc_client::RpcClient, GetInfoRequestMessage, RustweavedRequest, RustweavedResponse},
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(RustweavedPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type RustweavedRequestSender = async_channel::Sender<RustweavedRequest>;
type RustweavedRequestReceiver = async_channel::Receiver<RustweavedRequest>;
...
/// Data flow:
/// ```
/// //   RustweavedRequest -> request_send -> stream -> RustweavedResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: RustweavedRequestSender,
    request_receiver: RustweavedRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<RustweavedResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual RustweavedRequest to RustweavedResponse stream
        let mut stream: Streaming<RustweavedResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: RustweavedPayloadOps, request: impl Into<RustweavedRequest>) -> Result<RustweavedResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: RustweavedRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<RustweavedResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = rustweaved_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = rustweaved_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
