...
impl Inner {
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        // log_trace!("Rustweave wRPC::{encoding} connecting to: {url}");
                ...
                workflow_rpc::client::Notification::new(move |notification: kaspa_rpc_core::Notification| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: Rustweave RPC notification is not consumed by user: {:?}", notification);
                        }
                        Ok(())
                    })
                }),
...
impl Debug for Inner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RustweaveRpcClient")
            .field("rpc", &"rpc")
...
/// [`RustweaveRpcClient`] allows connection to the Rustweave wRPC Server via
...
///
#[derive(Clone)]
pub struct RustweaveRpcClient {
    inner: Arc<Inner>,
}

impl Debug for RustweaveRpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RustweaveRpcClient").field("url", &self.url()).field("connected", &self.is_connected()).finish()
    }
}

impl RustweaveRpcClient {
    /// Create a new `RustweaveRpcClient` with the given Encoding and URL
    pub fn new(
        encoding: Encoding,
        url: Option<&str>,
    ...
    ) -> Result<RustweaveRpcClient> {
        Self::new_with_args(encoding, url, resolver, network_id, subscription_context)
        // FIXME
        // pub fn new(encoding: Encoding, url: &str, ) -> Result<RustweaveRpcClient> {
    ...
    pub fn new_with_args(
        ...
        let client = RustweaveRpcClient { inner };
        ...
        // let client = RustweaveRpcClient { inner, notifier, notification_mode };
            ...
            loop {
                select! {
                    ...
                    msg = notification_relay_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            // inner.rpc_ctl.notify(msg).await.expect("(RustweaveRpcClient) rpc_ctl.notify() error");
                    ...
                    msg = wrpc_ctl_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            match msg {
                                WrpcCtl::Connect => {
                                    inner.rpc_ctl.signal_open().await.expect("(RustweaveRpcClient) rpc_ctl.signal_open() error");
                                }
                                WrpcCtl::Disconnect => {
                                    inner.rpc_ctl.signal_close().await.expect("(RustweaveRpcClient) rpc_ctl.signal_close() error");
                                }
                            }
                        } else {
...
#[async_trait]
impl RpcApi for RustweaveRpcClient {
    //
