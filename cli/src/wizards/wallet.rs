use crate::cli::RustweaveCli;
use crate::imports::*;
use crate::result::Result;
use kaspa_bip32::{Language, Mnemonic, WordCount};
use kaspa_wallet_core::storage::{make_filename, Hint};

pub(crate) async fn create(ctx: &Arc<RustweaveCli>, name: Option<&str>, import_with_mnemonic: bool) -> Result<()> {
    let term = ctx.term();
    ...
    if let Err(err) = wallet.network_id() {
        tprintln!(ctx);
        tprintln!(ctx, "Before creating a wallet, you need to select a Rustweave network.");
    ...
    if !import_with_mnemonic {
        ...
        tpara!(
            ...
            the Rustweave stored in it. Keep your mnemonic safe. Write it down and \
