// Basic example of a Rustweave wRPC client that connects to a node
...
struct Inner {
    ...
    // Rustweave wRPC client instance
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Rustweave RPC client instance using Borsh encoding.
        let client = Arc::new(RustweaveRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
