...
struct RpcTable {
    ...
    rustweaved_request_type: Expr,
    rustweaved_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, RustweavedRequestType, RustweavedResponseType, RustweavedPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let rustweaved_request_type = iter.next().unwrap().clone();
        let rustweaved_response_type = iter.next().unwrap().clone();
        ...
        Ok(RpcTable {
            ...
            server_ctx_type,
            connection_ctx_type,
            rustweaved_request_type,
            rustweaved_response_type,
            payload_ops,
            handlers,
        })
    }
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let rustweaved_request_type = &self.rustweaved_request_type;
        let rustweaved_response_type = &self.rustweaved_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #rustweaved_request_type, #rustweaved_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #rustweaved_request_type| {
                                Box::pin(async move {
                                    let mut response: #rustweaved_response_type = match request.payload {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #rustweaved_request_type, #rustweaved_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #rustweaved_request_type| {
                                Box::pin(async move {
                                    let mut response: #rustweaved_response_type = match request.payload {
