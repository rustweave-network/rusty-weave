...
impl Mempool {
    pub(crate) fn check_transaction_standard_in_isolation(&self, transaction: &MutableTransaction) -> NonStandardResult<()> {
        ...
        // However, in a later version of Rustweave the consensus-valid transaction version range might diverge from the
...
mod tests {
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        let dummy_tx_out = TransactionOutput::new(SOMPI_PER_RUSTWEAVE, dummy_script_public_key);
