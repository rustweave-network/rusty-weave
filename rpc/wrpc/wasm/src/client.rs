...
///
///
/// Rustweave RPC client uses ([wRPC](https://github.com/workflow-rs/workflow-rs/tree/master/rpc))
/// interface to connect directly with Rustweave Node. wRPC supports
/// two types of encodings: `borsh` (binary, default) and `json`.
///
/// There are two ways to connect: Directly to any Rustweave Node or to a
...
/// ```
///
/// **Connecting to a Rustweave Node directly**
///
/// ```javascript
/// let rpc = new RpcClient({
...
/// to re-register event listeners, but your have to re-subscribe for Rustweave node
/// notifications:
///
/// ```typescript
/// rpc.addEventListener("connect", async (event) => {
...
impl RpcClient {
    pub fn new(config: Option<RpcConfig>) -> Result<RpcClient> {
        ...
        let client = Arc::new(
            RustweaveRpcClient::new(encoding, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
...
impl RpcClient {
    ...
    /// Connect to the Rustweave RPC server. This function starts a background
    ...
    /// Disconnect from the Rustweave RPC server.
    ...
    /// however, you have to re-subscribe for Rustweave node notifications. As such, it is recommended
...
impl RpcClient {
    pub fn new_with_rpc_client(client: Arc<RustweaveRpcClient>) -> RpcClient {
    ...
    pub fn client(&self) -> &Arc<RustweaveRpcClient> {
        &self.inner.client
    }
...
impl RpcClient {
    ...
    pub fn parse_url(url: &str, encoding: Encoding, network: NetworkId) -> Result<String> {
        let url_ = RustweaveRpcClient::parse_url(url.to_string(), encoding, network.into())?;
        Ok(url_)
    }
}
...
impl RpcClient {
    ...
    /// Difficulty Adjustment Algorithm (DAA) score changes in the Rustweave BlockDAG.
    ...
    /// Rustweave BlockDAG. The event notification will be scoped to the
    ...
    /// chain changes in the Rustweave BlockDAG.
...
build_wrpc_wasm_bindgen_subscriptions!([
    ...
    /// block is added to the Rustweave BlockDAG.
    BlockAdded,
    ...
    /// conflict occurs in the Rustweave BlockDAG.
    FinalityConflict,
    ...
    /// conflict in the Rustweave BlockDAG is resolved.
    ...
    /// score of the sink block changes in the Rustweave BlockDAG.
    ...
    /// UTXO set override for the pruning point changes in the Rustweave BlockDAG.
    ...
    /// template is generated for mining in the Rustweave BlockDAG.
    NewBlockTemplate,
]);
...
build_wrpc_wasm_bindgen_interface!(
    [
        ...
        /// Retrieves the current number of blocks in the Rustweave BlockDAG.
        ...
        /// structure of the Rustweave BlockDAG.
        ...
        GetBlockDagInfo,
        /// Returns the total current coin supply of Rustweave network.
        /// Returned information: Total coin supply.
        GetCoinSupply,
        /// Retrieves information about the peers connected to the Rustweave node.
        /// Returned information: Peer ID, IP address and port, connection
        /// status, protocol version.
        GetConnectedPeerInfo,
        /// Retrieves general information about the Rustweave node.
        /// Returned information: Version of the Rustweave node, protocol
        /// version, network identifier.
        /// This call is primarily used by gRPC clients.
        /// For wRPC clients, use {@link RpcClient.getServerInfo}.
        GetInfo,
        /// Provides a list of addresses of known peers in the Rustweave
        ...
        GetSinkBlueScore,
        /// Tests the connection and responsiveness of a Rustweave node.
        /// Returned information: None.
        Ping,
        /// Gracefully shuts down the Rustweave node.
        /// Returned information: None.
        Shutdown,
        /// Retrieves information about the Rustweave server.
        /// Returned information: Version of the Rustweave server, protocol
        /// version, network identifier.
        GetServerInfo,
        /// Obtains basic information about the synchronization status of the Rustweave node.
        /// Returned information: Syncing status.
        GetSyncStatus,
    ],
    [
        // functions with `request` argument
        /// Adds a peer to the Rustweave node's list of known peers.
        /// Returned information: None.
        AddPeer,
        /// Bans a peer from connecting to the Rustweave node for a specified duration.
        /// Returned information: None.
        Ban,
        ...
        /// Retrieves the balance of a specific address in the Rustweave BlockDAG.
        /// Returned information: Balance of the address.
        GetBalanceByAddress,
        /// Retrieves balances for multiple addresses in the Rustweave BlockDAG.
        /// Returned information: Balances of the addresses.
        GetBalancesByAddresses,
        /// Retrieves a specific block from the Rustweave BlockDAG.
        /// Returned information: Block information.
        GetBlock,
        /// Retrieves multiple blocks from the Rustweave BlockDAG.
        ...
        GetCurrentNetwork,
        /// Retrieves block headers from the Rustweave BlockDAG.
        /// Returned information: List of block headers.
        GetHeaders,
        /// Retrieves mempool entries from the Rustweave node's mempool.
        ...
        GetMempoolEntry,
        /// Retrieves information about a subnetwork in the Rustweave BlockDAG.
        ...
        /// Resolves a finality conflict in the Rustweave BlockDAG.
        /// Returned information: None.
        ResolveFinalityConflict,
        /// Submits a block to the Rustweave network.
        /// Returned information: None.
        SubmitBlock,
        /// Submits a transaction to the Rustweave network.
        /// Returned information: None.
        SubmitTransaction,
        /// Unbans a previously banned peer, allowing it to connect
        /// to the Rustweave node again.
        /// Returned information: None.
        Unban,
    ]
);
