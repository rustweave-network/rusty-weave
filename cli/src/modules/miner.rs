...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<RustweaveCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<RustweaveCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<RustweaveCli>) -> Result<()> {
        let root = nw_sys::app::folder();
