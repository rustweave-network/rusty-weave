...
#[derive(Clone, Copy)]
struct Rustweave(f64);

impl Debug for Rustweave {
...
impl From<Rustweave> for Sompi {
    fn from(kaspa: Rustweave) -> Self {
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Rustweave(20.0),
            output_count: 1,
...
fn test_generator_sweep_two_utxos_with_priority_fees_rejection() -> Result<()> {
    let generator =
        make_generator(test_network_id(), &[10.0, 10.0], &[], Fees::sender(Rustweave(5.0)), change_address, PaymentDestination::Change);
    match generator {
...
fn test_generator_compound_200k_10kas_transactions() -> Result<()> {
    generator(test_network_id(), &[10.0; 200_000], &[], Fees::sender(Rustweave(5.0)), [(output_address, Rustweave(190_000.0))].as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, Rustweave(total - 10.0))];
    generator(test_network_id(), &inputs, &[], Fees::sender(Rustweave(5.0)), outputs.as_slice()).unwrap().harness().validate().finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, Rustweave(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], Fees::sender(Rustweave(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        &[10.0; 20],
        &[],
        Fees::sender(Rustweave(5.0)),
        [(output_address, Rustweave(1.0)), (output_address, Rustweave(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Rustweave(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Rustweave(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        &[],
        Fees::sender(Rustweave(5.0)),
        [(output_address, Rustweave(10.0)), (output_address, Rustweave(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Rustweave(5.0)), [(output_address, Rustweave(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Rustweave(0.0)), [(output_address, Rustweave(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: Rustweave(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Rustweave(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(Rustweave(5.0)),
            priority_fees: FeesExpected::sender(Rustweave(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[1.0; 100],
        &[],
        Fees::receiver(Rustweave(5.0)),
        // [(output_address, Rustweave(100.0))].as_slice(),
        [(output_address, Rustweave(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Rustweave(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Rustweave(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(Rustweave(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Rustweave(5.0)), [(output_address, Rustweave(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_903_outputs_2_fees_exclude() -> Result<()> {
    generator(test_network_id(), &[10.0; 1_000], &[], Fees::sender(Rustweave(5.0)), [(output_address, Rustweave(9_000.0))].as_slice())
        .unwrap()
        .harness()
        .drain(
            10,
        ...
        .fetch(&Expected {
            is_final: false,
            input_count: 21,
            aggregate_input_value: Rustweave(210.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
