...
use kaspa_grpc_core::{
    protowire::{
        ...
        RustweavedRequest, RustweavedResponse,
    },
...
impl Rpc for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn Stream<Item = Result<RustweavedResponse, tonic::Status>> + Send + Sync + 'static>>;

    /// Handle the new arriving client connection
    async fn message_stream(
        &self,
        request: Request<tonic::Streaming<RustweavedRequest>>,
